...
const CFG = Object.freeze({
  ...
  SECTION_SHEET: 'SurveySections',
  RULE_SHEET: 'QuestionRules',
  BANK_SHEET: 'QuestionBank',
  AUDIT_SHEET: 'AuditLog',
  ADMIN_USERS_SHEET: 'AdminUsers',
...
function setup() {
  ...
  ensureSheet_(ss, CFG.SECTION_SHEET, ['SectionId','SurveyId','Title','Description','SortOrder','Active','UpdatedAt']);
  ensureSheet_(ss, CFG.RULE_SHEET, ['RuleId','SurveyId','QuestionId','WhenQuestionId','Operator','CompareValue','Action','TargetQuestionId','Active','SortOrder','UpdatedAt']);
  ensureSheet_(ss, CFG.BANK_SHEET, ['BankId','Code','Label','Type','RequiredDefault','OptionsJson','HelpText','Active','CreatedAt','UpdatedAt']);
  ensureSheet_(ss, CFG.AUDIT_SHEET, ['AuditId','At','Username','Action','EntityType','EntityId','DetailsJson']);
  ensureSheet_(ss, CFG.ADMIN_USERS_SHEET, ['AdminId','Username','DisplayName','Role','Active','PasswordHash','PasswordSalt','CreatedAt','UpdatedAt']);
  ...
  if (getSheet_(CFG.ADMIN_USERS_SHEET).getLastRow() <= 1) {
    const salt=randomHex_(32);
    getSheet_(CFG.ADMIN_USERS_SHEET).appendRow([
      'ADM_'+Utilities.getUuid().slice(0,8).toUpperCase(),'admin','System Administrator','SUPER_ADMIN',true,
      hashPassword_('ChangeMe@123',salt),salt,new Date(),new Date()
    ]);
  }
...
function doPost(e) {
  try {
    ...
    switch(a) {
      ...
      case 'adminCloneSurvey': return adminCloneSurvey_(b);
      case 'adminSaveSection': return adminSaveSection_(b);
      case 'adminSaveRule': return adminSaveRule_(b);
      case 'adminDeleteRule': return adminDeleteRule_(b);
      case 'adminListBank': return adminListBank_(b);
      case 'adminSaveBank': return adminSaveBank_(b);
      case 'adminAddBankQuestion': return adminAddBankQuestion_(b);
      case 'adminReport': return adminReport_(b);
      case 'adminListAdmins': return adminListAdmins_(b);
      case 'adminSaveAdmin': return adminSaveAdmin_(b);
...
function getSurvey_(b) {
  ...
  const out=publicSurvey_(survey);
  out.sections=getSectionsPlatform_(survey.SurveyId);
  out.questions=getQuestions_(survey.SurveyId);
  out.rules=getRulesPlatform_(survey.SurveyId);
  return json_({ok:true,survey:out});
}
...
function adminLogin_(b) {
  const modern = findModernAdmin_(String(b.username||'').trim());
  if (modern) {
    const pw=String(b.password||'');
    if (!isTrue_(modern.Active) || !secureEqual_(hashPassword_(pw,modern.PasswordSalt),modern.PasswordHash))
      return json_({ok:false,error:'INVALID_LOGIN',message:'Tài khoản hoặc mật khẩu không đúng.'});
    const token=createSession_('admin',{username:modern.Username,displayName:modern.DisplayName,role:modern.Role});
    auditPlatform_(modern.Username,'LOGIN','Admin',modern.AdminId,{role:modern.Role});
    return json_({ok:true,token,user:{username:modern.Username,displayName:modern.DisplayName,role:modern.Role}});
  }
...
function platformRole_(token){
  const s=getSession_(token);
  return s&&s.type==='admin'?s.payload:null;
}
function canEditPlatform_(s){return s&&['SUPER_ADMIN','ADMIN','EDITOR'].indexOf(s.role)>=0;}
function canReportPlatform_(s){return s&&['SUPER_ADMIN','ADMIN','EDITOR','REPORT_VIEWER'].indexOf(s.role)>=0;}
function platformUnauthorized_(){return json_({ok:false,error:'UNAUTHORIZED',message:'Phiên Admin đã hết hạn.'});}
function platformForbidden_(){return json_({ok:false,error:'FORBIDDEN',message:'Không đủ quyền.'});}

function adminListSurveys_(b){
  const s=platformRole_(b.token); if(!s)return platformUnauthorized_();
  return json_({ok:true,surveys:getSurveysPlatform_()});
}
function getSurveysPlatform_(){
  const sh=getSheet_(CFG.SURVEY_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return[];
  const hm=headerMap_(sh),out=[];
  for(let i=1;i<rows.length;i++){const o={_row:i+1};Object.keys(hm).forEach(k=>o[k]=rows[i][hm[k]]);out.push(o);}
  return out.map(x=>({SurveyId:x.SurveyId,Code:x.Code,Name:x.Name,Title:x.Title,Description:x.Description,ConsentText:x.ConsentText,Status:x.Status,StartAt:dateISO_(x.StartAt),EndAt:dateISO_(x.EndAt),Version:Number(x.Version||1),UpdatedAt:dateISO_(x.UpdatedAt)}));
}
function adminCreateSurvey_(b){
  const s=platformRole_(b.token);if(!canEditPlatform_(s))return platformForbidden_();
  const x=b.survey||{},code=clean_(x.Code,80).replace(/\s+/g,'_').toUpperCase();
...
function adminCloneSurvey_(b){
  ...
  qs.forEach(q=>{const nid='Q_'+Utilities.getUuid().slice(0,8).toUpperCase();qmap[q.QuestionId]=nid;getSheet_(CFG.QUESTION_SHEET).appendRow([nid,id,map[q.SectionId]||'',q.Code,q.Label,q.Type,q.Required,q.Placeholder,q.HelpText,JSON.stringify(q.Validation||{}),q.SortOrder,true,now,now]);});
  getRulesPlatform_(src.SurveyId).forEach(r=>getSheet_(CFG.RULE_SHEET).appendRow(['R_'+Utilities.getUuid().slice(0,8).toUpperCase(),id,qmap[r.QuestionId]||'',qmap[r.WhenQuestionId]||'',r.Operator,r.CompareValue,r.Action,qmap[r.TargetQuestionId]||'',true,r.SortOrder,now]));
  auditPlatform_(s.username,'CLONE','Survey',id,{source:src.Code,newCode:code});
  return json_({ok:true,code});
}
function adminSaveSection_(b){
  const s=platformRole_(b.token);if(!canEditPlatform_(s))return platformForbidden_();
  const survey=findSurveyByCode_(String(b.code||''));if(!survey)return json_({ok:false,error:'NOT_FOUND'});
  const x=b.section||{},now=new Date(),sh=getSheet_(CFG.SECTION_SHEET);
  if(x.SectionId){
    const hm=headerMap_(sh),row=findRowBy_(sh,hm.SectionId,x.SectionId);if(row<2)return json_({ok:false,error:'NOT_FOUND'});
    [['Title',x.Title],['Description',x.Description],['SortOrder',Number(x.SortOrder)||999],['Active',x.Active!==false],['UpdatedAt',now]].forEach(a=>sh.getRange(row,hm[a[0]]+1).setValue(a[1]));
  }else{
    sh.appendRow(['SEC_'+Utilities.getUuid().slice(0,8).toUpperCase(),survey.SurveyId,clean_(x.Title,500),clean_(x.Description,3000),Number(x.SortOrder)||999,true,now]);
  }
  return json_({ok:true});
}
function getSectionsPlatform_(surveyId){
  const sh=getSheet_(CFG.SECTION_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return[];
  const hm=headerMap_(sh),out=[];for(let i=1;i<rows.length;i++)if(String(rows[i][hm.SurveyId])===String(surveyId)&&isTrue_(rows[i][hm.Active]))out.push({SectionId:rows[i][hm.SectionId],SurveyId:rows[i][hm.SurveyId],Title:rows[i][hm.Title],Description:rows[i][hm.Description],SortOrder:Number(rows[i][hm.SortOrder]||999)});return out.sort((a,b)=>a.SortOrder-b.SortOrder);
}
function adminSaveRule_(b){
  const s=platformRole_(b.token);if(!canEditPlatform_(s))return platformForbidden_();
  const survey=findSurveyByCode_(String(b.code||''));if(!survey)return json_({ok:false,error:'NOT_FOUND'});
  const x=b.rule||{},sh=getSheet_(CFG.RULE_SHEET),hm=headerMap_(sh),now=new Date();
  if(x.RuleId){
    const row=findRowBy_(sh,hm.RuleId,x.RuleId);if(row<2)return json_({ok:false,error:'NOT_FOUND'});
    [['WhenQuestionId',x.WhenQuestionId],['Operator',x.Operator],['CompareValue',x.CompareValue],['Action',x.Action],['TargetQuestionId',x.TargetQuestionId],['Active',x.Active!==false],['UpdatedAt',now]].forEach(a=>sh.getRange(row,hm[a[0]]+1).setValue(a[1]));
  }else sh.appendRow(['R_'+Utilities.getUuid().slice(0,8).toUpperCase(),survey.SurveyId,x.QuestionId,x.WhenQuestionId,x.Operator,x.CompareValue,x.Action,x.TargetQuestionId,true,Number(x.SortOrder)||999,now]);
  return json_({ok:true});
}
function adminDeleteRule_(b){
  const s=platformRole_(b.token);if(!canEditPlatform_(s))return platformForbidden_();
  const sh=getSheet_(CFG.RULE_SHEET),hm=headerMap_(sh),row=findRowBy_(sh,hm.RuleId,String(b.ruleId||''));if(row<2)return json_({ok:false,error:'NOT_FOUND'});
  sh.getRange(row,hm.Active+1).setValue(false);return json_({ok:true});
}
function getRulesPlatform_(surveyId){
  const sh=getSheet_(CFG.RULE_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return[];
  const hm=headerMap_(sh),out=[];for(let i=1;i<rows.length;i++)if(String(rows[i][hm.SurveyId])===String(surveyId)&&isTrue_(rows[i][hm.Active]))out.push({RuleId:rows[i][hm.RuleId],QuestionId:rows[i][hm.QuestionId],WhenQuestionId:rows[i][hm.WhenQuestionId],Operator:rows[i][hm.Operator],CompareValue:rows[i][hm.CompareValue],Action:rows[i][hm.Action],TargetQuestionId:rows[i][hm.TargetQuestionId],SortOrder:Number(rows[i][hm.SortOrder]||999)});return out.sort((a,b)=>a.SortOrder-b.SortOrder);
}
function adminListBank_(b){
  const s=platformRole_(b.token);if(!s)return platformUnauthorized_();
  const sh=getSheet_(CFG.BANK_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return json_({ok:true,items:[]});
  const hm=headerMap_(sh),items=[];for(let i=1;i<rows.length;i++)if(isTrue_(rows[i][hm.Active]))items.push({BankId:rows[i][hm.BankId],Code:rows[i][hm.Code],Label:rows[i][hm.Label],Type:rows[i][hm.Type],Options:parseJson_(rows[i][hm.OptionsJson],[]),HelpText:rows[i][hm.HelpText]});return json_({ok:true,items});
}
function adminSaveBank_(b){
  const s=platformRole_(b.token);if(!canEditPlatform_(s))return platformForbidden_();
  const x=b.item||{},sh=getSheet_(CFG.BANK_SHEET),hm=headerMap_(sh),id=x.BankId||'BANK_'+Utilities.getUuid().slice(0,8).toUpperCase(),row=x.BankId?findRowBy_(sh,hm.BankId,x.BankId):-1,now=new Date();
  const vals=[id,clean_(x.Code||id,100),clean_(x.Label,1000),String(x.Type||'text'),x.RequiredDefault===true,JSON.stringify(x.Options||[]),clean_(x.HelpText,2000),x.Active!==false,now,now];
  if(row<2)sh.appendRow(vals);else sh.getRange(row,1,1,vals.length).setValues([vals]);return json_({ok:true});
}
function adminAddBankQuestion_(b){
  const s=platformRole_(b.token);if(!canEditPlatform_(s))return platformForbidden_();
  const bankId=String(b.bankId||''),bank=adminListBankInternal_(bankId),survey=findSurveyByCode_(String(b.code||''));if(!bank||!survey)return json_({ok:false,error:'NOT_FOUND'});
  const id='Q_'+Utilities.getUuid().slice(0,8).toUpperCase(),now=new Date();
  getSheet_(CFG.QUESTION_SHEET).appendRow([id,survey.SurveyId,b.sectionId||'',bank.Code,bank.Label,bank.Type,bank.RequiredDefault,'',bank.HelpText,'{}',999,true,now,now]);
  return json_({ok:true,questionId:id});
}
function adminListBankInternal_(id){
  const sh=getSheet_(CFG.BANK_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return null;const hm=headerMap_(sh);for(let i=1;i<rows.length;i++)if(String(rows[i][hm.BankId])===id)return {BankId:id,Code:rows[i][hm.Code],Label:rows[i][hm.Label],Type:rows[i][hm.Type],RequiredDefault:isTrue_(rows[i][hm.RequiredDefault]),Options:parseJson_(rows[i][hm.OptionsJson],[]),HelpText:rows[i][hm.HelpText]};return null;
}
function adminReport_(b){
  const s=platformRole_(b.token);if(!canReportPlatform_(s))return platformForbidden_();
  const survey=findSurveyByCode_(String(b.code||''));if(!survey)return json_({ok:false,error:'NOT_FOUND'});
  const answers=getAnswersForPlatform_(survey.SurveyId),qs=getQuestions_(survey.SurveyId),result=qs.map(q=>{const c={};answers.filter(a=>a.QuestionId===q.QuestionId).forEach(a=>{let v=a.ValueJson?parseJson_(a.ValueJson,a.ValueJson):a.ValueText||a.ValueNumber||'';(Array.isArray(v)?v:[v]).forEach(x=>c[x]=(c[x]||0)+1)});return {QuestionId:q.QuestionId,Code:q.Code,Label:q.Label,Total:answers.filter(a=>a.QuestionId===q.QuestionId).length,Counts:c};});
  return json_({ok:true,result});
}
function getAnswersForPlatform_(surveyId){
  const sh=getSheet_(CFG.RESPONSE_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return[];
  const hm=headerMap_(sh),out=[];for(let i=1;i<rows.length;i++)if(String(rows[i][hm.SurveyId])===String(surveyId)){let a=parseJson_(rows[i][hm.AnswersJson],{});Object.keys(a).forEach(q=>out.push({QuestionId:q,ValueText:Array.isArray(a[q])?'':String(a[q]),ValueJson:Array.isArray(a[q])?JSON.stringify(a[q]):''}));}return out;
}
function adminListAdmins_(b){
  const s=platformRole_(b.token);if(!s||s.role!=='SUPER_ADMIN')return platformForbidden_();
  const sh=getSheet_(CFG.ADMIN_USERS_SHEET),rows=sh.getDataRange().getDisplayValues();if(rows.length<2)return json_({ok:true,items:[]});
  const hm=headerMap_(sh),items=[];for(let i=1;i<rows.length;i++)items.push({AdminId:rows[i][hm.AdminId],Username:rows[i][hm.Username],DisplayName:rows[i][hm.DisplayName],Role:rows[i][hm.Role],Active:isTrue_(rows[i][hm.Active])});return json_({ok:true,items});
}
function adminSaveAdmin_(b){
  const s=platformRole_(b.token);if(!s||s.role!=='SUPER_ADMIN')return platformForbidden_();
  const x=b.user||{},sh=getSheet_(CFG.ADMIN_USERS_SHEET),hm=headerMap_(sh),now=new Date(),u=clean_(x.Username,100).toLowerCase();if(!u)return json_({ok:false,error:'INVALID_USERNAME'});
  const row=findRowBy_(sh,hm.Username,u),salt=randomHex_(32);
  if(row<2)sh.appendRow(['ADM_'+Utilities.getUuid().slice(0,8).toUpperCase(),u,clean_(x.DisplayName,200),x.Role||'REPORT_VIEWER',x.Active!==false,hashPassword_(String(x.Password||'ChangeMe@123'),salt),salt,now,now]);
  else {if(x.DisplayName!==undefined)sh.getRange(row,hm.DisplayName+1).setValue(x.DisplayName);if(x.Role)sh.getRange(row,hm.Role+1).setValue(x.Role);if(x.Active!==undefined)sh.getRange(row,hm.Active+1).setValue(x.Active);if(x.Password){sh.getRange(row,hm.PasswordHash+1).setValue(hashPassword_(String(x.Password),salt));sh.getRange(row,hm.PasswordSalt+1).setValue(salt)}sh.getRange(row,hm.UpdatedAt+1).setValue(now)}
  return json_({ok:true});
}
function findModernAdmin_(u){
  const sh=getSheet_(CFG.ADMIN_USERS_SHEET),rows=sh.getDataRange().getValues();if(rows.length<2)return null;const hm=headerMap_(sh);
  for(let i=1;i<rows.length;i++)if(String(rows[i][hm.Username]||'').toLowerCase()===u.toLowerCase()){const o={};Object.keys(hm).forEach(k=>o[k]=rows[i][hm[k]]);return o;}return null;
}
function auditPlatform_(u,a,t,id,d){try{getSheet_(CFG.AUDIT_SHEET).appendRow(['AUD_'+Utilities.getUuid().slice(0,8).toUpperCase(),new Date(),u,a,t,id,JSON.stringify(d||{})])}catch(e){}}
